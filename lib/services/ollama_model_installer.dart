...
class OllamaModelInstaller {
  ...
  Future<ModelArtifact> install({
    ...
    final manifestResponse = await _client.get(
      ...
      headers: const {'Accept': 'application/vnd.docker.distribution.manifest.v2+json'},
    );
    ...
    if (response.statusCode != 200) {
      throw HttpException('Ollama model download failed: ${response.statusCode}', uri: blobUri);
    }
    ...
    try {
      await for (final chunk in response.stream) {
        ...
        final denominator = expectedSize > 0 ? expectedSize : response.contentLength ?? 1;
        onProgress(
    ...
    if (actualSize != expectedSize) {
      ...
      throw StateError('Model size mismatch: expected $expectedSize, got $actualSize.');
    }

    onProgress(0.99, 'Verifying SHA-256');
    final actualDigest = await sha256.bind(partial.openRead()).first;
    final expectedDigest = digestHex;
