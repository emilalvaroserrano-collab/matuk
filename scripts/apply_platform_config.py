...
# Flutter templates use build.gradle.kts on current stable.
kts = root / 'android/app/build.gradle.kts'
if kts.exists():
    text = kts.read_text()
    text = re.sub(r'minSdk\s*=\s*flutter\.minSdkVersion', 'minSdk = 26', text)
    if 'ndkVersion' in text:
        text = re.sub(r'ndkVersion\s*=\s*flutter\.ndkVersion', 'ndkVersion = "27.0.12077973"', text)
    else:
    ...
    if gradle.exists():
        ...
        if 'ndkVersion' in text:
            text = re.sub(r'ndkVersion\s+flutter\.ndkVersion', 'ndkVersion "27.0.12077973"', text)
...
text = re.sub(r"^\s*#?\s*platform :ios, '[^']+'", "platform :ios, '16.4'", text, flags=re.MULTILINE)
...
# Add minimum deployment target and permission_handler microphone macro.
...
# Xcode target version is also patched to 16.4 for consistency.
pbx = root / 'ios/Runner.xcodeproj/project.pbxproj'
if pbx.exists():
    text = pbx.read_text()
    text = re.sub(r'IPHONEOS_DEPLOYMENT_TARGET = [0-9.]+;', 'IPHONEOS_DEPLOYMENT_TARGET = 16.4;', text)
    pbx.write_text(text)

print('Applied Android/iOS local-inference platform configuration.')
